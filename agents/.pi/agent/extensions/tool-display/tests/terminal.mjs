...
const editor = executable("vim", 'exec /usr/bin/vim -u NONE -i NONE -n -c "syntax on" "$@"');
...
try {
  ...
  while (!existsSync(join(root, "background-done")) && Date.now() < deadline) await delay(25);
  ...
  screen = await terminal.waitFor(
    (s) => s.includes("BACKGROUND_FINISHED") && s.includes("BUSY_DRAFT_KEEP_ME"),
    "return to Pi restores draft and background results",
  );
  ...
  terminal.assertColour("✗", colours.red);
  ...
  terminal.save();
  console.log(
    `PASS: xterm.js VT + real Pi/Vim; groups, Unicode clicks, colours/underlines, gated background work, draft/save, images, errors, Ctrl+O, resize, reload/resume, bounded top paging/anchors, archived Vim/images, unchanged context. Pi idle CPU ${cpu.toFixed(2)}s/2s. Artifacts: ${root}`,
  );
} catch (error) {
  terminal?.save();
